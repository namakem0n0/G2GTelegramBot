...
namespace TelegramBotExperiments
{
    class Program
    {
        static ITelegramBotClient bot = new TelegramBotClient("6801419152:AAGFETbc_vtTNFe1FGWutMuZ34wk6AOoP0I");
        ...
        public static async Task HandleUpdateAsync(ITelegramBotClient botClient, Update update, CancellationToken cancellationToken)
        {
            if(update.Type == Telegram.Bot.Types.Enums.UpdateType.Message)
            {
                ...
                {
                    await botClient.SendTextMessageAsync(message.Chat.Id, "Type /start to start bot");
                    return;
                }

                if (message.Text.ToLower() == "/start")
                {
                    await botClient.SendTextMessageAsync(message.Chat.Id, "Lutsiv`s personal bot for G2G!");
        
                    // Prompt user for their login
                    await botClient.SendTextMessageAsync(message.Chat.Id, "Please provide your login for G2G:");
